import type { AgUiEvent, AgUiInterrupt, AgUiRunFinishedOutcome } from "./types.js";
...
export const parseAgUiEvent = (
  ...
  const getAgentName = () => {
    ...
    if (isPlainObject(rawEvent) && isString(rawEvent.name)) {
      return rawEvent.name;
    }
    return getString("name");
  };

  const parsed = ((): AgUiEvent | null => {
    switch (typeValue) {
      case "RUN_STARTED": {
      ...
      case "TEXT_MESSAGE_START":
        return withOptional(
          ...
          { messageId: getString("messageId"), agentName: getAgentName() },
        );
